#!/usr/bin/env python3
"""Deterministic, read-only full repository audit."""
...
def _kind(path: Path) -> str:
    ...
    if "requirements" in path.name.lower() and path.suffix == ".txt": return "requirements"
    return "text"
...
def _python_findings(rel: str, text: str) -> tuple[list[Finding], dict[str, int]]:
    findings: list[Finding] = []
    metrics = {"functions": 0, "classes": 0, "max_complexity": 0, "max_function_lines": 0}
    try:
        tree = ast.parse(text, filename=rel)
    except SyntaxError as exc:
        return [Finding("critical", "PY-SYNTAX", rel, int(exc.lineno or 1), str(exc))], metrics
        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name):
                ...
                if owner in {"pickle", "dill"} and name in {"load", "loads"}:
                    findings.append(Finding("high", "PY-UNSAFE-DESERIALIZE", rel, node.lineno, f"{owner}.{name} can execute untrusted input"))
            ...
            if isinstance(value, ast.Constant) and isinstance(value.value, str):
                ...
                literal = value.value
                is_env_name = any(name.endswith("_ENV") or name.endswith("_ENV_NAME") for name in names) and bool(ENV_NAME_LITERAL.fullmatch(literal))
                if names and any(SENSITIVE_NAME.search(name) for name in names) and len(literal) >= 12 and not PLACEHOLDER.search(literal) and not is_env_name:
...
def audit(root: Path) -> dict[str, Any]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if any(part in IGNORED for part in rel_path.parts): continue
        ...
        for index, line in enumerate(text.splitlines(), 1):
            ...
            if rel != "tools/repository_audit.py" and re.search(r"\b(TODO|FIXME|HACK|XXX)\b", line, re.I):
                findings.append(Finding("medium", "TXT-DEBT-MARKER", rel, index, line.strip()[:220]))
            if rel != "tools/repository_audit.py" and "a15280020511/test" in line and Path(rel).name not in HISTORICAL:
            ...
            if kind == "yaml" and re.match(r"\s*uses:\s*", line):
                ...
                if not action.startswith(("./", "docker://")) and not PINNED_ACTION.fullmatch(action):
                    findings.append(Finding("high", "GHA-UNPINNED-ACTION", rel, index, f"action is not pinned to a full commit SHA: {action}"))
        if kind == "python":
            py_findings, py_metrics = _python_findings(rel, text)
            findings.extend(py_findings); metrics[rel] = py_metrics
        elif kind == "json":
            try: json.loads(text)
            except json.JSONDecodeError as exc: findings.append(Finding("critical", "JSON-PARSE", rel, exc.lineno, exc.msg))
        elif kind == "requirements":
            requirements[rel] = text
    for digest, paths in hashes.items():
        useful = [p for p in paths if not p.endswith(("__init__.py", ".gitkeep"))]
        if len(useful) > 1:
            findings.append(Finding("low", "FILE-DUPLICATE", useful[0], 1, f"identical content shared by {useful}; sha256={digest}"))
    ...
    for rel, text in requirements.items():
        for index, raw in enumerate(text.splitlines(), 1):
            value = raw.strip()
            if not value or value.startswith("#") or value.startswith("-r "): continue
            match = REQUIREMENT.fullmatch(value)
            if not match:
                findings.append(Finding("medium", "REQ-UNPINNED", rel, index, f"dependency is not constrained: {value}")); continue
    ...
    return {"schema_version": "repository-audit-v4", "file_count": len(files), "total_lines": sum(item["line_count"] or 0 for item in files), "finding_counts": {name: counts.get(name, 0) for name in SEVERITY}, "findings": [asdict(item) for item in findings], "files": files, "python_metrics": metrics}


def _markdown(report: dict[str, Any]) -> str:
    rows = ["# Full Repository Audit", "", f"- Files: `{report['file_count']}`", f"- Lines inspected: `{report['total_lines']}`", *(f"- {name.title()}: `{report['finding_counts'][name]}`" for name in SEVERITY), "", "| Severity | Rule | File | Line | Finding |", "|---|---|---|---:|---|"]
...
def main() -> int:
    ...
    for item in report["findings"][:50]:
        ...
        print(f"::{level} file={item['path']},line={item['line']}::{item['severity']} {item['rule']}: {item['message']}")
