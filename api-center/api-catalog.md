...
- 托管操作总数：`123`
- 已公开参数总数：`768`
- 目录 SHA-256：`62e6370fbce3611d20fb0c1d27dc7aff7fb421f18dccccdcbb4b5dc38e8ec259`
- 选择者：`GPTs 使用中心`
...
| China-Check 免费中国企业基础核验 | `china-check` | 启用 | `[api-prc-open]` | `3` | 否 |
| SinoFacts 中国科技与出海企业开放事实库 | `sinofacts` | 启用 | `[api-prc-open]` | `3` | 否 |
...
## China-Check 免费中国企业基础核验 (`china-check`)

- 状态：`启用`
- 说明：通过固定的无认证 MCP 只读端点查询中国大陆企业基础登记信息；仅保留企业级公开字段，自动剔除个人联系信息和付费升级字段。
- 目录策略：只允许固定 MCP 端点和两个公开只读工具；不得提交任意 URL、Cookie、Authorization、代理、脚本或付费操作。
- 执行策略：每张票据最多执行一次 company-search 或 company-snapshot；遇到 401/403/429/CAPTCHA/WAF/区域拒绝立即停止，不换 IP、账号、工具或执行出口重试。China-Check 仅作为可替换的基础企业核验源，不作为唯一事实来源。
- 票据前缀：`[api-prc-open]`
- Secret环境变量名：`无`（仅名称）
- Repository Variable名：`无`（仅名称）
- 提供方SHA-256：`f2be1288edfb13494315f725edf3728de702da1b8b38985ca606c245b946454e`

| 操作 | 说明 | 参数 |
|---|---|---|
| `catalog-capabilities` | 读取 China-Check 本地安全能力目录，不访问上游。 | `无` |
...
| `company-search` | 按企业名称、品牌、域名、电话或统一社会信用代码搜索中国大陆企业；输出基础企业匹配并移除个人字段。 | `query, language` |
...
| `company-snapshot` | 读取单家公司免费的基础登记快照；仅输出公开企业字段，不自动购买或进入付费报告。 | `query, company_id, language` |

`company-snapshot` 参数Schema：
...
## SinoFacts 中国科技与出海企业开放事实库 (`sinofacts`)

- 状态：`启用`
- 说明：读取 SinoFacts 在 GitHub 发布的 CC BY 4.0 企业事实快照，包含来源、验证日期和置信度；适用于科技、软件与出海企业的证据增强。
- 目录策略：仅读取固定 GitHub Raw 文件 index.json 与 companies.jsonl；禁止任意仓库、任意 URL、写操作和全库再分发。
- 执行策略：按企业名称、slug 或公开关键词进行有界本地过滤；每次最多返回 20 个匹配或 1 个企业记录，保留来源和许可证信息。
- 票据前缀：`[api-prc-open]`
- Secret环境变量名：`无`（仅名称）
- Repository Variable名：`无`（仅名称）
- 提供方SHA-256：`2c1583a81166e0b0a2311fd21c21d1840300bbd07d785470f1fab64cf7157e1b`

| 操作 | 说明 | 参数 |
|---|---|---|
| `catalog-capabilities` | 读取 SinoFacts 本地安全能力目录，不访问上游。 | `无` |
...
| `company-search` | 在 SinoFacts 开放索引中搜索企业并返回有界匹配。 | `query, max_results` |
...
| `company-profile` | 按 slug 或企业名称在 SinoFacts JSONL 快照中返回单个完整公开记录，并附来源和 CC BY 4.0 归属信息。 | `query` |

`company-profile` 参数Schema：
