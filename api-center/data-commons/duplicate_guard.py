#!/usr/bin/env python3
"""Reject duplicate [api-dc] task IDs or equivalent ticket bodies across Issues."""
...
def _api_json(url: str, token: str) -> Any:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "managed-data-commons-api-center",
        },
    )
    ...
    for page in range(1, 6):
        ...
        for row in rows:
            ...
            if issue_number == current_issue:
                continue
            if not str(row.get("title") or "").startswith("[api-dc]"):
                continue
            try:
