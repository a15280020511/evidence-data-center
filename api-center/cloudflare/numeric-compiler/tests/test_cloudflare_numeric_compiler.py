...
SPEC = importlib.util.spec_from_file_location(
    "cloudflare_numeric_compiler_runtime",
...
class CloudflareNumericCompilerTests(unittest.TestCase):
    ...
    def test_configuration_is_fixed_and_complete(self) -> None:
        ...
        self.assertEqual(receipt["domain_count"], 12)
    ...
    def test_link_result_converts_to_numeric_link(self) -> None:
        ...
        self.assertNotEqual(
            rows[0]["source_entity_id"], rows[0]["target_entity_id"]
        )
    ...
    def test_ticket_rejects_unknown_profile_and_private_target(self) -> None:
        ...
        with patch.object(MODULE.socket, "getaddrinfo", return_value=records):
            with self.assertRaisesRegex(
                MODULE.NumericCompilerError, "not allowlisted"
            ):
    ...
    def test_prepare_persists_ticket_but_not_page_content(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            event_path.write_text(
                json.dumps({"issue": {"body": json.dumps(ticket)}}),
                encoding="utf-8",
            )
            with patch.object(MODULE.socket, "getaddrinfo", return_value=records):
                receipt = MODULE.prepare(event_path, root / "out")
            self.assertEqual(
                receipt["status"], "CLOUDFLARE_NUMERIC_TICKET_ACCEPTED"
            )
            self.assertFalse(receipt["raw_text_persisted"])
            self.assertFalse((root / "out" / "raw-page.txt").exists())

    def test_failure_receipt_is_rendered_over_admission(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "admission-receipt.json").write_text(
                json.dumps(
                    {
            ...
            (root / "failure-receipt.json").write_text(
                json.dumps(
                    {
                        "status": "CLOUDFLARE_NUMERIC_COMPILATION_FAILED",
                        "error_type": "NumericCompilerError",
            ...
            self.assertIn("COMPILATION_FAILED", rendered)
            self.assertNotIn("TICKET_ACCEPTED", rendered)
