...
catalog_path = AGT / "provider-catalog.json"
catalog = json.loads(catalog_path.read_text(encoding="utf-8"))
provider = catalog["providers"][0]
before = {row["operation_id"] for row in provider["operations"]}
assert STOCK_OPS <= before, sorted(STOCK_OPS - before)
provider["operations"] = [
    row for row in provider["operations"] if row["operation_id"] not in STOCK_OPS
]
after = {row["operation_id"] for row in provider["operations"]}
assert len(after) == 21
assert not (STOCK_OPS & after)
provider["display_name"] = "Agent Toolbelt 通用代理工具"
provider["description"] = (
    "通过 Agent Toolbelt 官方 API 调用20项通用代理工具；"
    "不开放其仅限美股的8项股票研究工具，也不开放任何写操作。"
)
provider["catalog_policy"] = (
    "固定开放21项能力：1项本地目录和20项官方通用工具；"
    "每个票据最多一次受限请求，参数、正文、数组、文本、图片、URL、"
    "超时和响应体积均受硬上限约束。"
)
provider["execution_policy"] = (
    "API Key仅由GitHub Repository Secret注入；不接受客户端凭据、任意工具名、"
    "任意路径、股票研究、Watchlist CRUD、交易、下单、Webhook或后台监控。"
    "URL型工具仅接受公开HTTPS地址并拒绝本机、私网和保留地址字面量。"
)
provider["limits"]["fixed_upstream_tool_count"] = 20
provider["limits"]["stock_research_tools_allowed"] = False
provider["limits"]["us_equity_research_allowed"] = False
write_json(catalog_path, catalog)

schema_path = AGT / "ticket.schema.json"
schema = json.loads(schema_path.read_text(encoding="utf-8"))
schema["properties"]["operation"]["enum"] = [
    op for op in schema["properties"]["operation"]["enum"] if op not in STOCK_OPS
]
assert len(schema["properties"]["operation"]["enum"]) == 21
assert not STOCK_OPS.intersection(schema["properties"]["operation"]["enum"])
write_json(schema_path, schema)

(AGT / "README.md").write_text(
...
(AGT / "tests" / "test_agent_toolbelt_task.py").write_text(
    '''from __future__ import annotations
...
replacements = {
    ROOT / "tests" / "test_api_catalog.py": [
        ('"agent-toolbelt": 29', '"agent-toolbelt": 21'),
        ('catalog["managed_operation_count"], 368', 'catalog["managed_operation_count"], 360'),
        ('len(agent_toolbelt["operations"]), 29', 'len(agent_toolbelt["operations"]), 21'),
    ],
    ...
    Path(".github/workflows/api-catalog-validate.yml"): [
        ("== 368", "== 360"),
        ("len(agent_toolbelt['operations']) == 29", "len(agent_toolbelt['operations']) == 21"),
        ("'managed_operations': 368", "'managed_operations': 360"),
        ("'agent_toolbelt_operations': 29", "'agent_toolbelt_operations': 21"),
    ],
}
...
api_test = ROOT / "tests" / "test_api_catalog.py"
text = api_test.read_text(encoding="utf-8")
needle = '        self.assertEqual(len(agent_toolbelt["operations"]), 21)\n'
addition = (
    needle
    + '        self.assertFalse(\n'
...
api_test.write_text(text.replace(needle, addition, 1), encoding="utf-8")

for path in (ROOT / "README.md", ROOT / "SECRET_ISOLATION_POLICY.md"):
    text = path.read_text(encoding="utf-8")
    lines: list[str] = []
    for line in text.splitlines():
        if "Agent Toolbelt" in line or "agent-toolbelt" in line:
            line = line.replace("AI 股票研究与代理工具", "通用代理工具")
            line = line.replace("29 项", "21 项").replace("29项", "21项")
            line = line.replace("28 项固定上游工具", "20 项固定上游通用工具")
            line = line.replace("28项固定上游工具", "20项固定上游通用工具")
            line = line.replace("8 项美股研究和 20 项通用工具", "20 项通用工具")
            line = line.replace("8项美股研究和20项通用工具", "20项通用工具")
        lines.append(line)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")

final_validation = '''name: Validate Agent Toolbelt Provider

on:
  pull_request:
    paths:
      - "api-center/agent-toolbelt/**"
      ...
      - ".github/workflows/agent-toolbelt-provider-validate.yml"
      - ".github/workflows/agent-toolbelt-api-ticket.yml"
  push:
    branches: [main]
    paths:
      - "api-center/agent-toolbelt/**"
    ...
    steps:
      ...
      - uses: actions/setup-python@a26af69be951a213d495a4c3e4e4022e16d87065 # v5
        with:
          python-version: "3.12"
          cache: pip
          cache-dependency-path: api-center/agent-toolbelt/requirements.txt

      - name: Install dependencies
        run: |
          python -m pip install --disable-pip-version-check --no-input -r api-center/agent-toolbelt/requirements.txt
          python -m pip check

      - name: Compile and test provider
        run: |
          python -m py_compile api-center/build_catalog.py api-center/managed_provider_runtime.py api-center/agent-toolbelt/agent_toolbelt_task.py api-center/agent-toolbelt/tests/*.py
          python -m unittest discover -s api-center/agent-toolbelt/tests -p 'test_*.py' -v

      - name: Validate fixed contracts
        run: |
          python - <<'PY'
          import json
          from pathlib import Path
          ...
          root = Path('api-center/agent-toolbelt')
          schema = json.loads((root / 'ticket.schema.json').read_text(encoding='utf-8'))
          catalog = json.loads((root / 'provider-catalog.json').read_text(encoding='utf-8'))
          Draft202012Validator.check_schema(schema)
          provider = catalog['providers'][0]
          operation_ids = {row['operation_id'] for row in provider['operations']}
          assert provider['provider_id'] == 'agent-toolbelt'
          assert provider['required_secret_environment_variable'] == 'AGENT_TOOLBELT_KEY'
          assert len(operation_ids) == 21
          assert not stock_operations.intersection(operation_ids)
          assert 'context-window-packer' in operation_ids
          assert 'create-watchlist' not in operation_ids
          assert provider['limits']['fixed_upstream_tool_count'] == 20
          assert provider['limits']['stock_research_tools_allowed'] is False
          assert provider['limits']['us_equity_research_allowed'] is False
          assert provider['limits']['fixed_api_host'] == 'www.agenttoolbelt.live'
          assert provider['limits']['arbitrary_tool_names_allowed'] is False
          assert provider['limits']['watchlist_crud_allowed'] is False
          ...
          assert provider['limits']['secret_values_exposed'] is False
          print(json.dumps({'status': 'PASS', 'operations': len(operation_ids), 'stock_operations': 0}))
          PY

      - name: Verify repository contains no credential material
        run: |
          git diff --check
          ! git grep -nE 'atb_[A-Za-z0-9_-]{12,}' -- ':!api-center/agent-toolbelt/tests/**'
'''
Path(".github/workflows/agent-toolbelt-provider-validate.yml").write_text(
    final_validation, encoding="utf-8"
)

for temporary in (
    Path(".github/workflows/remove-agent-toolbelt-stock-tools.yml"),
    Path(".github/scripts/remove_agent_toolbelt_stock_tools.py"),
):
    if temporary.exists():
        temporary.unlink()
